#!/usr/bin/env bash
# quick_bench.sh - Benchmark rápido con límite reducido para validación
# Uso: bash scripts/quick_bench.sh [RUNS]

set -euo pipefail

RUNS=${1:-2}
LIMIT=4194304  # 2^22 (mucho más rápido para testing)
BINARY="./bruteforce"
OUTCSV="results/quick_benchmark.csv"
...
}' "$OUTCSV" > results/quick_benchmark_summary.csv

echo ""
echo "=== RESUMEN ==="
column -t -s',' results/quick_benchmark_summary.csv
